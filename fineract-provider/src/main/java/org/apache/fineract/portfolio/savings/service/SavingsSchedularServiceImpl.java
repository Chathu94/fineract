...
//        long start = System.currentTimeMillis();
        ...
        List<Long> savingsAccounts = this.savingsAccountRepository.findAccountIdsByStatus(ACTIVE.getValue());
        LOG.info(savingsAccounts.size() + " accounts found");
        for (final Long savingsAccount : savingsAccounts) {
            LOG.info("Processing Account " + savingsAccount);
            this.savingsAccountWritePlatformService.calculateInterest(savingsAccount);

            Long savingsId = savingsAccount;
            final boolean postInterestAs = false;
            final LocalDate transactionDate = DateUtils.getLocalDateOfTenant().minusDays(1);
            ...
            savingsAccountWritePlatformService.postInterest(account, postInterestAs, transactionDate, backdatedTxnsAllowedTill);
        }
